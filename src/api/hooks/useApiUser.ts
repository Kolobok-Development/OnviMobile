...
function useGetMe() {
  const {data, error, isLoading} = useSWR<ReturnType<typeof getMe>>(
    'me',
    fetcher.bind(null, getMe),
  );
  return {data, error, isLoading: isLoading};
}

function useGetTariff() {
  const {data, error, isLoading} = useSWR<ReturnType<typeof getTariff>>(
    'tariff',
    fetcher.bind(null, getTariff),
  );
  return {data, error, isLoading: isLoading};
}
...
function useGetOrderHistory(params: IGetAccountHistoryRequestParams) {
  const {data, error, mutate, isLoading} = useSWR<
    ReturnType<typeof getOrderHistory>
  >(
    params ? ['order-hist', params] : null,
    fetcher.bind(null, getOrderHistory, params),
  );
...
function useGetCampaignHistory() {
  const {data, error, mutate, isLoading} = useSWR<
    ReturnType<typeof getCampaignHistory>
  >('campaign-hist', fetcher.bind(null, getCampaignHistory));
  return {data, error, isLoading: isLoading, mutate};
}

function useUpdateUser() {
  const {mutate, isLoading, error} = useSWR<ReturnType<typeof update>>(
    'update-user',
    update,
    {
      onError: err => {
