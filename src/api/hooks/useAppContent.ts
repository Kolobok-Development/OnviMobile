...
function usePartner(id: number) {
  const {data, error, isLoading} = useSWR(
    id ? ['partner', id] : null,
    fetcher.bind(null, getPartner, id),
  );
...
function useCampaign(id: number) {
  const {data, error, isLoading} = useSWR(
    id ? ['campaign', id] : null,
    fetcher.bind(null, getCampaign, id),
  );
  return {data, error, isLoading: isLoading};
}
...
function useCampaigns() {
  const {data, error, isLoading} = useSWR(
    'campaign',
    fetcher.bind(null, getCampaigns),
  );
  return {data, error, isLoading: isLoading};
}
...
function useBusiness(query: {[key: string]: string}, enabled?: boolean) {
  const {data, isLoading, error, mutate} = useSWR(
    enabled ? ['business', query] : null,
    fetcher.bind(null, getCarWashes, query),
  );
...
function useNewsPosts() {
  const {data, error, isLoading} = useSWR(
    'news',
    fetcher.bind(null, getNewsPosts),
  );
  return {data, error, isLoading: isLoading};
}
...
function useNewsPost(id: number) {
  const {data, error, isLoading} = useSWR(
    id ? ['news', id] : null,
    fetcher.bind(null, getNewsPost, id),
  );
  return {data, error, isLoading: isLoading};
}
