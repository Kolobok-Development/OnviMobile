...
import {GestureResponderEvent, PanResponder, PanResponderGestureState, StyleSheet, Text, View, Animated as RNAnimated} from "react-native";
...
const SumInput: React.FC<ISumInput>= ({
    ...
    const _calculateValue = (gestureState: PanResponderGestureState) => {
        ...
        if (step) {
            const newValue = Math.max(minValue, Math.min(
                maxValue, _moveStartValue.value + Math.round((ratio * diff) / step ) * 10
            ));
            return newValue
        }else {
            return Math.floor(Math.max(minValue, _moveStartValue.value + ratio * diff) * 100
            ) / 100;
        }
    }
    ...
    const sliderStyle = useAnimatedStyle(() => {
        const _newHeight = ((_value.value - minValue + 50) * height - 50) / (maxValue - minValue);
