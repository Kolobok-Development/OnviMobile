...
const Search = () => {
  ...
  const doSearch = useCallback(debounce(async (val: string) => {
     await getBusinesses()
  }, 1300), [])
  ...
  const onClick = (carwash: any) => {
    ...
    route.params.cameraRef.current.moveTo([carwash.location.lon, carwash.location.lat], 1)
  }
        ...
        <BottomSheetScrollView style={styles.container} nestedScrollEnabled={true}>
            <View style={{ flex: 1,  flexDirection: 'row', borderRadius: dp(30)}}>
                ...
                <TextInput
                    ...
                    style={{ backgroundColor: "rgba(245, 245, 245, 1)", borderRadius: dp(30), width: "85%", height: dp(40), paddingLeft: dp(18), textAlign: 'left', fontSize: dp(16), color: "#000000" }}
                />
            </View>
            <View>
              {isLoading || isFetching ? <SearchPlaceholder /> : (
                <>
                  {!data || !data.businessesLocations || data.businessesLocations.length === 0  ? (
                    <View style={{ display: "flex", justifyContent: "center", flex: 1, alignItems: "center" }}>
                      <Text style={{ marginTop: dp(20), fontSize: dp(15) }}>Мойки не найдены</Text>
                    </View>
                  ) : (
                      <View>
                          <FlatList
                              ...
                              keyExtractor={(item: CarWashLocation, index: number) => index.toString()}
                          />
                      </View>
                  )}
                </>
