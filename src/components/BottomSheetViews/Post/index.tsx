...
import {useNewsPost, useNewsPosts} from "../../../api/hooks/useAppContent";
                    ...
                    <View style={{display: 'flex', flexDirection: "column"}}>
                        <View style={{paddingTop: dp(15), paddingBottom: dp(10)}}>
                            ...
                            {!post ? <PostPlaceholder /> : (
                                <>
                                    <Image source={{ uri: post.attributes.image.data.attributes.url }} style={{width: "100%", flex: 1, aspectRatio: post.attributes.image.data.attributes.width / post.attributes.image.data.attributes.height, borderRadius: dp(25) }} />
                                    <View style={{display: "flex", flexDirection: "column", padding: dp(16)}}>
                                        <Text style={{ fontSize: dp(22), fontWeight: "700", color: "#000" }}>{post.attributes.title}</Text>
                                        {/* @ts-ignore */}
                                        <Markdown
                                            style={{
                                        ...
                                        <View style={{justifyContent: 'space-evenly', alignItems: "center", flex: 1, paddingTop: dp(20), paddingBottom: dp(100)}}>
                                            {
                                                post.attributes.button_title && (
                                                    <Button label={post.attributes.button_title} width={200} onClick={() => {
                                                        if (post.attributes.url) {
                                                            Linking.openURL(post.attributes.url);
                                                        } else if (post.attributes.screen_redirect) {
                                                            //@ts-ignore
                                                            navigation.navigate(post.attributes.screen_redirect, route.params)
                                                        }
                                                    }} color='blue' />
                                                )
                                            }
